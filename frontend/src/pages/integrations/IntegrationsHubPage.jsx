...
export default function IntegrationsHubPage() {
  ...
  const getIcon = (category) => {
    if (category.includes('Credential')) return <ShieldCheck className="w-6 h-6 text-emerald-600" />;
    if (category.includes('Learning')) return <BookOpen className="w-6 h-6 text-violet-600" />;
    if (category.includes('Assessment')) return <Code2 className="w-6 h-6 text-indigo-600" />;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-slate-500 font-mono">Automated Webhooks & APIs</span>
        </div>
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-slate-900">{integ.name}</h3>
                    <p className="text-xs text-slate-600 mt-1 max-w-2xl">{integ.description}</p>
                  </div>
                </div>

                <div className="flex flex-col items-start md:items-end gap-2 shrink-0">
                  <span className="text-xs font-mono text-slate-500">
                    Records Synced: <strong className="text-slate-900 text-sm">{integ.recordsSynced?.toLocaleString()}</strong>
                  </span>
                  <button
                    onClick={() => handleSync(integ._id, integ.name)}
                    disabled={syncingId === integ._id}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-700 text-white transition-all text-xs font-semibold shadow-sm shadow-violet-500/20 disabled:opacity-50"
                  >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div className="flex flex-wrap items-center gap-1.5">
                  <span className="text-slate-500 font-semibold mr-1">Synchronized Data:</span>
                  {integ.supportedDataTypes?.map((dt) => (
                    <span
                      key={dt}
                      className="px-2.5 py-0.5 rounded-md bg-slate-100 text-slate-700 border border-slate-200 font-mono text-[11px]"
                    >
                      {dt}
                    </span>
                  ))}
