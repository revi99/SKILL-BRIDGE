    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs sm:text-sm font-semibold px-4 py-2.5 rounded-xl shadow-md shadow-purple-500/20 transition-all hover:scale-105 shrink-0"
        >
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 rounded-3xl border border-slate-800 text-center space-y-3">
          ...
          <h3 className="text-base font-bold text-white">No Proposals Published Yet</h3>
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Create an FDP or research call to invite industry experts to your department.
          </p>
          <button
            onClick={() => setShowModal(true)}
            className="px-4 py-2 bg-purple-600 text-white rounded-xl text-xs font-semibold mt-2"
          >
            Create Proposal
          </button>
        </div>
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
                <div>
                  ...
                  <h3 className="text-lg font-bold text-white mt-1.5">{collab.title}</h3>
                </div>
                ...
                <div className="pt-3 border-t border-slate-800/80 space-y-2">
                  ...
                  <div className="space-y-2">
                    {collab.interests.map((interest, idx) => (
                      <div
                        key={idx}
                        className="p-3 rounded-xl bg-slate-900 border border-emerald-500/20 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                      >
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <strong className="text-white">{interest.companyName}</strong>
                            <span className="text-slate-400">({interest.recruiterName})</span>
                          </div>
                          <p className="text-slate-300 italic">"{interest.message}"</p>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="glass-panel max-w-lg w-full p-6 sm:p-8 rounded-3xl border border-slate-700 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-lg font-bold text-white">Post Collaboration Proposal</h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">Target Tech Domain</label>
                <input
                  type="text"
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white text-xs font-semibold shadow-md shadow-purple-600/30"
                >
